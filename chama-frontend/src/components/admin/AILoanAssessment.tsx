...
export function AILoanAssessment({ chamaId, loanId, onEvaluationLoaded, compact = false }: AILoanAssessmentProps) {
  ...
  return (
    <Card className="border-slate-200">
      <div
        className="flex cursor-pointer items-center justify-between border-b border-slate-100 px-4 py-3"
        onClick={() => setExpanded((e) => !e)}
      >
        <div className="flex items-center gap-2">
          <Sparkles size={18} className="text-blue-600" />
          <span className="font-semibold text-slate-800">AI Loan Assessment</span>
          {evaluation && (
        ...
        {!evaluation ? (
          ...
          <button type="button" className="p-1 text-slate-500">
            ...
            <>
              <div className="grid gap-4 sm:grid-cols-2">
                <div>
                  ...
                  <p className="text-2xl font-bold text-slate-800">{evaluation.riskScore}/100</p>
                </div>
                <div>
                  <p className="text-xs font-medium uppercase text-slate-500">Recommendation</p>
                  <p className="font-semibold text-slate-800">{evaluation.recommendation}</p>
                </div>
              </div>
              <div>
                <p className="mb-1 text-xs font-medium uppercase text-slate-500">Key metrics</p>
              ...
              {evaluation.reasons.length > 0 && (
                <div>
                  <p className="mb-1 text-xs font-medium uppercase text-slate-500">Risk factors</p>
              ...
              {evaluation.aiExplanation && (
                <div>
                  <p className="mb-1 text-xs font-medium uppercase text-slate-500">AI explanation</p>
                  <p className="text-sm text-slate-700">{evaluation.aiExplanation}</p>
                </div>
              )}
            </>
          )}
